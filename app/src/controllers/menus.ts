...
export const UI = {
  ...
  openParty(): void {
    ...
    Game.party.forEach((m) => {
      ...
      h += `<div class="card" style="margin:6px 0;text-align:left">
        ...
        <div class="small" style="margin-top:6px">Abilities: ${unlockedSkills(m).length}/${m.skills.length} unlocked <button class="btn" style="padding:1px 8px;font-size:11px" onclick="UI.skillTree('${m.id}')">Skill Tree ▸</button></div>
      </div>`;
    });
  ...
  skillTree(memberId: string): void {
    ...
    kit.forEach((s) => {
      ...
      h += `<div class="card" style="margin:6px 0;text-align:left;border-color:${ok ? (s.ult ? "var(--legendary)" : "var(--gold)") : "var(--line)"};${ok ? "" : "opacity:.6"}">
        <div><b class="${ok ? (s.ult ? "r-legendary" : "r-uncommon") : ""}">${s.name}</b>${s.ult ? " ★" : ""} <span class="pill">${tree} ${s.mnaReq}</span> ${gate}</div>

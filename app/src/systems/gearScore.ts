...
type Scorable = Pick<Member, "atk" | "mag" | "critPct" | "spd" | "leech" | "maxhp" | "armor" | "maxmp">;

/** Three composite scores from a member's effective stats (integers). */
export function gearScore(m: Scorable): GearScore {
  const offense = Math.round(m.atk + m.mag + m.critPct * 1.5 + m.spd * 0.4 + (m.leech || 0) * 0.6);
  const defense = Math.round(m.maxhp * 0.12 + m.armor * 5 + m.maxmp * 0.08);
